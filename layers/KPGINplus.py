...
class KPGINPlusConv(MessagePassing):
    ...
    def __init__(self, input_size, output_size, K, num_hop1_edge=1, num_pe=1, combine="independent"):
        ...
        if self.K > 1:
            ...
            self.hopk_node_path_emb = torch.nn.Linear(num_pe, input_size)#torch.nn.Embedding(num_pe, input_size, padding_idx=0)
            if self.K==3:
                self.hopk_node_path_emb = torch.nn.Linear(2, input_size)
    ...
    def forward(self, x, edge_index, edge_attr, pe_attr=None, peripheral_attr=None):
        ...
        x_n = self.propagate(edge_index, x=x)  # N * K * H
