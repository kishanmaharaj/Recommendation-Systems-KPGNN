...
class KPGINConv(MessagePassing):
    ...
    def __init__(self, input_size, output_size, K, eps=0., train_eps=False, num_hop1_edge=1, num_pe=1,
        ...
        if self.K > 1:
            ...
            self.hopk_node_path_emb = torch.nn.Linear(48, self.input_dk)#torch.nn.Embedding(num_pe, self.input_dk, padding_idx=0)
            if self.K==3:
                self.hopk_node_path_emb = torch.nn.Linear(48, 16)
    ...
    def forward(self, x, edge_index, edge_attr, pe_attr=None, peripheral_attr=None):
        ...
        # e1_emb = self.hop1_edge_emb(torch.ones()).unsqueeze(dim=1)
        # print("e1_emb", e1_emb.shape)
        if self.K > 1:
            if pe_attr is not None:
                pe = self.hopk_node_path_emb(pe_attr)
                x[:, 1:] = x[:, 1:] + pe.view(pe.shape[0], 1,pe.shape[1])

        #     # ek_emb = self.hopk_edge_emb(edge_attr[:, 1:])  # E * K-1 * dk
        #     ek_emb = self.hopk_edge_emb(torch.arange(0, edge_index.size(1), device = x.device)).unsqueeze(dim=1)
        #     e_emb = torch.cat([e1_emb, ek_emb], dim=-2)  # E * K * dk
        # else:
        #     e_emb = e1_emb
        
        # print("e_emb", e_emb.shape)

        # x_n = self.propagate(edge_index, x=x, edge_emb=e_emb, mask=edge_attr)  # N * K * dk
        x_n = self.propagate(edge_index, x=x)
